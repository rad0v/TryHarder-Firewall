...
def block_ip(ip):
    """Block the given IP using iptables and redirect to port 8080 where the web server will handle the redirect to YouTube."""
    if is_ip_blocked(ip):
    ...
    try:
        ...
        # Redirect all traffic from that IP on port 80 to port 8080
        subprocess.run(["sudo", "iptables", "-t", "nat", "-A", "PREROUTING", "-s", ip, "-p", "tcp", "--dport", "80", "-j", "REDIRECT", "--to-port", "8080"], check=True)

        print(f"IP {ip} will be redirected to the YouTube video on port 8080.")
